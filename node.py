...
def OmDet_detect(data, labels, conf_threshold=0.30, nms_threshold=0.5,device="cuda"):
...
class ApplyOmDet:
    ...
    def main(
        ...
        results = OmDet_detect(data, labels, conf_threshold, nms_threshold, device)
        
        res_images = []
        res_masks = []
        res_labels = []
        
        for image_pil, result in zip(data, results):
            size = image_pil.size
            pred_dict = {
                "size": [size[1], size[0]],
                "result":result
