...
class DetEngine(BaseEngine):
    ...
    def __init__(self, model_dir="OmDet", device="cpu", batch_size=10):
    ...
    def _init_cfg(self, cfg, model_id):
        model_path = os.path.join(folder_paths.models_dir, self.model_dir)
        cfg.MODEL.WEIGHTS = os.path.join(model_path, model_id + ".pth")
    ...
    def _load_model(self, model_id):
        if not self._models.has(model_id):
            ...
            config_path = os.path.join(current_file_path, "configs", model_id + ".yaml")
